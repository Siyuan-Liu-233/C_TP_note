    ...
    for(i=1;i<(image_p2.hauter-1);i++)
    	{
    		for(j=1;j<(image_p2.largeur-1);j++)
    	    	{
    	    		*(p+k)=(-1)*(image_p2.donner[i-1][j-1]+image_p2.donner[i+1][j-1])+1*(image_p2.donner[i-1][j+1]+image_p2.donner[i+1][j+1])-2*image_p2.donner[i][j-1]+2*image_p2.donner[i][j+1];
    	    		k++;
    	    	}
    	}
